...
$previous_avg_ask = ""; 
while(true){
	$sql = "SELECT avg(ask) FROM " . MYSQL_TRD_DB . "." . MYSQL_TRD_MAIN_TABLE . " WHERE name like '%EURUSD%' and datetime like '" . $strdtday . "%'";
	$r = mysqli_query($connloc, $sql);
	...
	if ($r->num_rows>0){
		while($row = $r->fetch_assoc()){
			...
			if ($previous_avg_ask != ""){
				...
				$sql = "SELECT * FROM " . MYSQL_TRD_DB . "." . MYSQL_TRD_MAIN_TABLE . " WHERE name like '%EURUSD%' ORDER BY datetime DESC";
				...
				if ($avg_ask>$previous_avg_ask){
					echo $strdt . " EURUSD " . $avg_ask . " avg ask +++" . " ; last ask = " . $last_ask . " ; last bid = " . $last_bid . "  \r\n";
